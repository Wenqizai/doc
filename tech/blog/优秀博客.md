...
[Your Biggest Customer Might Be Your Biggest Bottleneck](https://densumesh.dev/blog/fair-queue/)<https://densumesh.dev/blog/fair-queue/>

不公平队列的调度，所有客户的 task 都放在同一个队列，FIFO：

问题描述：某一个客户 A 突增流量，导致队列充满客户 A 的 task，FIFO，一直调度处理客户 A 得 task，导致其他客户的 task 没有被调度处理，被挤压

**传统解决方案：**

1. 限制客户并发

- 对业务有损型，通常客户会要求下游系统来承担这个流量洪峰；
- 客户需要对限流增加，重试等复杂逻辑，下游系统需要增加限流逻辑。上下游的系统设计的复杂度都增加

2. 专用的队列和 worker

每个客户维护一个处理任务的队列和负责消费的 worker。避免某一个客户的突增流量挤压到其他客户的 task 消费。

这种设计通常是昂贵的，因为客户突增的流量是业务的一个常态，特性。当处理完这批 task 之后，worker 就处于空闲状态，维护是昂贵的。

**公平队列解决方案**

1. 调度队列 + worker，负责存储需要调度的 task 和处理 task；
2. 存储队列，一个客户一个队列，专职专用
